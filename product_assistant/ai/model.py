...
class VskAIModel(AIModel):
    ...
    def __init__(self):
        ...
        self._api_key = (
            settings.vsk_api_key.get_secret_value()
            if settings.vsk_api_key
            else ""
        )
    ...
    def response(self, query: str) -> str:
        for attempt in range(1, self.retries + 1):
            try:
                ...
                if (
                    self._is_empty_response(exc)
                    and attempt < self.empty_response_retries
                ):
                    print(
                ...
                raise RuntimeError(
                    f"Ошибка VSK AI API: таймаут после {self.retries} попыток"
                ) from exc
